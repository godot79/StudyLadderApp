...
**Rivers/mountains-specific follow-up (2026-08-18):** targeted search for a released exam archive covering physical geography (rivers, mountains, capitals) at native grade 3-5 level, to fill geography.json's thin rivers (14 mentions) / mountains (10 mentions) bands at Age 9/Age 10.

| Source | URL | Verdict | Notes |
|---|---|---|---|
| **International Geography Bee — Elementary Division, released rounds** | internationalgeographybee.com/usa/resources/ | 🔴 ruled out | Pulled and read an actual round (2022 Elementary Round 1, PDF). Despite the "Elementary" label, content is competitive quiz-bowl level — items like identifying the Basque Country from Euskara/Guernica/Vitoria-Gasteiz clues, or Qatar from Khawr al Udayd/Salwa Canal/al-Thani clues. This is not a "harder source, rewrite down" case like Virginia SOL was — the underlying facts themselves are too obscure for a 9-year-old, not just the phrasing. Also carries a real licensing caveat the archive sources don't: page footer reads "Copyright 2017 International Geography Bee. All Rights Reserved" — private competition IP, not a government-released public exam. |
| **State DOE / national-assessment archives (general search)** | — | 🔴 confirmed structural gap | No new lead found beyond what's already documented above (Virginia SOL, NY Regents SS). Reinforces the existing catalog finding: no US state or national system appears to run a standalone, released, elementary-grade (3-5) physical-geography exam archive. Rivers/mountains/capitals content at this age band lives in worksheets and trivia sites, not exam archives. |
| **TeachersPayTeachers free tier** | (URLs already listed above) | 🟡 unchanged, remains the best real lead | Re-confirmed via search: free worksheets exist specifically on "major rivers and mountains of the world" at the right grade band (5th-8th per one listing), consistent with the existing catalog note. Still a worksheet repository, not an exam archive — same caveats as before (inconsistent quality, no guaranteed answer key, needs per-item vetting at ingestion time). No new archive-quality alternative was found this round, so this is the fallback if the round proceeds without a better exam source. |

**Verdict (2026-08-18):** no exam-archive-quality source was found for rivers/mountains/capitals at grade 3-5. Every prior round in this pipeline drew from an official released exam. This is a genuine gap, not a research shortfall — flagged to the user rather than defaulting to TPT unprompted, since it's a lower-quality source than the pipeline has used so far.
